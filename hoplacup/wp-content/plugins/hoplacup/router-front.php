...
function hoplacup_template_redirect()
{
    ...
    /**
     * Le paramètre hoplacup_param=1 est utilisé comme un indicateur dans l'URL
     * pour signaler que la requête concerne la gestion des routes définies par le plugin Hoplacup.
     * Lorsque WordPress traite l'URL, il va vérifier la présence de ce paramètre pour déterminer
     * si la règle de réécriture associée à votre plugin doit être appliquée.
     */
    ...
    if ($hoplacup_param) {
        ...
        $page_parameters['url'] = $page_parameters['public_root_url'] = site_url() . '/tournois';
        ...
        switch ($rubrique) {
            ...
            case 'equipes':
                ...
                exit; // Arrêter l'exécution après avoir affiché le message
                break;
